#本模型是textRNN_half_ubuntu模型的第3版
#由于效果跟使用tsvd的textRNN_half系列差不多，训练速度也比前者快，所以用它来测试长序列的预测情况会不会更好
#序列长度为500,MLP第二层节点为500，优化器Adam，初始学习率0.01
#取消正交初始化，改为默认初始化
import os
import torch
...
class BisaiDataset(Dataset):#数据预处理器
    ...
    def csv2frame(self,filepath):#给出单场比赛的csv文件路径，并转化成帧列表和对应变帧时间列表，以及比赛结果
        ...
        lables = new_data[:,0]
        if len(frametimelist)>500:#如果总帧数大于250,则随机挑选250，若不足250,则在后面处理vectensor时补0
        ...
        if len_frame<500:
            vectensor = np.concatenate((np.zeros((500-len_frame,12),dtype=np.float64),vectensor),axis=0)#如果不足500，则在前面用0填充
...
class Lstm(nn.Module):#在模型建立之处就把它默认初始化
    def __init__(self):
        ...
        self.encoder = nn.LSTM(input_size=12, 
                                hidden_size=700,#选择对帧进行保留首尾的均匀截断采样
                                num_layers=1,#暂时就只有一层
                                bidirectional=True)
        self.decoder = nn.Sequential(
            nn.Linear(2000, 500),#把LSTM的输出
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(500, 3)
        )
...
if __name__ == "__main__":
    train_writer = SummaryWriter('/home/jsy/log5/train')#自动建立
    test_writer = SummaryWriter('/home/jsy/log5/test')#自动建立
    checkpoint_path = '/home/jsy/log5/checkpoint.pth'#ckpoint文件夹需要提前建立
