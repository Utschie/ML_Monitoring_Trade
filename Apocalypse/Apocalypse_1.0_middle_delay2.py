#本模型是基于1.0_middle_delay，针对随机探索做的改进
#在得到max_frametime之后，在0-max_frametime之间随机选取时间点，然后再在确定的时间点下进行交易
#而逐渐降低随机程度分成两部分，一个是通过减少随机选取的时间点，从125个直到0个，而动作随机的减少方式则是通过降低随机率————20201008
import os
            ...
            while True:
                ...
                if (step_counter >2000) and (step_counter%25 == 0) :#2000步之后每转移50次进行一次eval_Q的学习
                    ...
                    with tf.GradientTape() as tape:
                        ...
                        y_true = batch_revenue+max_Q_value*(1-np.array(batch_done))#由于延迟收益太难传回去，所以先不用gamma
                        #y_pred
