'''
即时收益+可变长度输入+终赔不参与投资+归一化+100万次转移转贪心+-100负收益+gamma(1.0)不折现+Adam(0.001,amsgrad =True)
'''
#这次加了amsgrad=True,不知道效果如何
import os
...
def jiangwei(state,capital,mean_invested):
    ...
    length = len(state)/410.0#出赔率的公司数
...
if __name__ == "__main__":
    start0 = time.time()
    summary_writer = tf.summary.create_file_writer('./tensorboard_0.4_middle_sofort2') #在代码所在文件夹同目录下创建tensorboard文件夹（本代码在jupyternotbook里跑，所以在jupyternotebook里可以看到）
    #########设置超参数
    ...
    weights_path = 'D:\\data\\eval_Q_weights_0.4_middle_sofort2.ckpt'
