...
#再或者就是把所有帧当成单个图片，就只用CNN做个分类，再用训练好的CNN的结果给每个帧进行标注，用标注后的lable再进行lstm或者传统方法对单场进行回归————20201219
#再或者就是textCNN，直接把每场比赛那么多帧压缩成一张(10,500)的图片，或者是保留初始和结尾，把一场比赛改成(3,10,601)的图片，然后直接用CNN处理————20201219
import os
import torch
