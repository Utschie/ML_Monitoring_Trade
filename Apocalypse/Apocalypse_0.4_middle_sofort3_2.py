...
#还是取最大q值吗？还是也用随机策略选择q值，还是基于概率计算平均q值？————20200828
#其实如果是回归概率的话可以用策略梯度方法，最后出一个采用各策略的概率————20200828
#而这个模型暂时还是不改loss函数，只是改选择策略的方法，虽然没什么理由，但是就简单试试看怎么样————20200828
#其实原模型回归完后可以采用随机策略的方式在测试集上测试————20200828
import os
