...
#middle程序的运行速度反而比mini要快一点点，可能是由于降维函数运行速度的微小差别————20201031
#或许beta的增长还可以再慢一些，原论文说是到训练结束时增加到1，那么也就是其实它可以再缩小个几倍————20201104
#而且就实际训练过程也可以发现，当beta达到1之后，actor的loss慢慢开始上升，而当beta到0.9时，看起来actor的loss的震荡比较小————20201104
#结束随机需要9天半的时间，在刚结束随机时同样会遇到方差很大的情况，需要让它再跑几天看看————20201106
#在critic学习了70000多次之后，actor的loss似乎触底上升
#可以试一下最新的middle_SAC3的critic的学了70000次的Q网络做一次测试，看看跟之前的学了不到40000次的Q网络有无差别————20201114
#70000次学习，0.2粒度的SAC3模型的随机策略测试结果虽然也就那样，略优于Sofort2，即在test1上表现差不多，但在test2上要差于纯随机，略优于sofort2————20201115
#如果只用SAC3的Q做确定性策略，它在test2上的收益稳定性和结果要优于之前所有的模型，而在test1上的表现也超过所有无论稳定性还是峰值都优于sofort2模型————20201115
#就是不知道为什么SAC随机策略测试结果的无行动率总为0，而确定策略new_SAC_Q倒是一直有无行动的时候————20201116
#唯一的解释就是随机策略里的net网络的参数和critic的Q网络参数大不相同以至于选不出0动作来，可能因为critic和actor的网络形式本来也有区别————20201116
#所以目前看来，细粒度的确定型策略可能更容易出效果————20201115
#所以确定型策略接下来要解决的问题，一个是用DDPG建立确定型策略模型，一个是把细粒度的策略模型迁移学习到粗粒度的策略模型上————20201115
#但整体来讲，随机策略不如人意，甚至不如确定性策略，当然这里是有由于策略的熵和reward的绝对值之间不太可比的可能性存在，但是还是先搁置吧————20201115
#强化学习的model-free模型应专注与跨期套利模型————20201115
        ...
        for j in filelist:#挨场比赛训练
            ...
            while True:
                ...
                if done:#终盘时储存信息，同时更新actor，清除actor内存
                    transition = np.array((state,action, revenue,jiangwei(next_state,next_capital,next_frametime,bianpan_env.mean_invested),1))
                    ...
                    transition = np.array((state,action, revenue,jiangwei(next_state,next_capital,next_frametime,bianpan_env.mean_invested),0))
