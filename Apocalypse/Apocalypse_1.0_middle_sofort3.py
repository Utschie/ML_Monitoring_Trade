...
#如果每层节点达到600的话，那么有150万个参数，300是40万个参数，400是70万个参数，500是100万个参数————20201009
#运行速度是每分钟293.6次转移，那么要想达到1500万次转移，需要35天，太长了————20201009
#但是也可以试一试————20201009
import os
...
if __name__ == "__main__":
    ...
    summary_writer4 = tf.summary.create_file_writer('./tensorboard_1.0_middle_sofort3/used_steps')
    summary_writer5 = tf.summary.create_file_writer('./tensorboard_1.0_middle_sofort3/bisai_steps')
    #########设置超参数
    ...
    target_weights_path = 'D:\\data\\target_Q_weights_1.0_middle_sofort3.ckpt'
            ...
            while True:
                ...
                if (step_counter >2000) and (step_counter%50 == 0) :#2000步之后每转移50次进行一次eval_Q的学习
                    ...
                    if (learn_step_counter % 300 == 0) and (learn_step_counter > 0):#每学习300次，target_Q网络参数进行一次变量替换
                        ...
                        target_Q.save_weights(target_weights_path, overwrite=True)#保存并覆盖之前的检查点，储存权重
